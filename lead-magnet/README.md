# Lead magnet — The Signal Valuation Lab (a Claude skill)

The free incentive that grows The Signal's email list: a **Claude skill** that
...
- It funnels: the free Lab is a taste; the **store** DCF/Monte-Carlo/dividend
  models are the paid upgrade, and **The Signal** is the recurring research.

## How to distribute it as a lead magnet (email-gated)

The skill is the reward for joining the free list:

1. **Zip the skill folder** (`the-signal-valuation-lab/`).
...
`~/.claude/skills/the-signal-valuation-lab/` (or a project's `.claude/skills/`).
...
```bash
python3 the-signal-valuation-lab/scripts/valuation.py --model all --json '{
